-- =============================================================================
-- DATAGOL 2026 — Migración de Base de Datos SQL: 49_process_call_completed_sentiment.sql
-- =============================================================================
-- Agrega `p_sentiment` a process_call_completed para persistir el análisis de
-- sentimiento de llamadas ("Positivo" / "Neutral" / "Negativo") en `call_logs.sentiment`.
--
-- Definición base copiada de db/migrations/41_process_call_completed_source.sql.
-- El upsert de call_logs (sección 2) persiste `sentiment` en el INSERT y lo actualiza
-- en el ON CONFLICT DO UPDATE con el patrón `COALESCE(EXCLUDED.sentiment, public.call_logs.sentiment)`:
-- una siembra inicial de llamada saliente sin sentimiento no pisa un sentimiento
-- capturado con posterioridad, y la llegada del webhook post-call enriquece la fila.
--
...
BEGIN
    ...
    -- 2. Upsert de call_logs por provider_call_id. AGREGA sentiment (esta migración).
    ...
    -- 3. Upsert de lead. Sin cambios (ver migración 41).
...
-- Backfill no destructivo: normaliza los registros históricos de call_logs sin sentimiento
UPDATE public.call_logs
SET sentiment = CASE
    WHEN duration_seconds >= 20 AND length(COALESCE(transcript, '')) > 50 THEN 'Positivo'
    ELSE 'Neutral'
END
WHERE sentiment IS NULL;

COMMENT ON FUNCTION public.process_call_completed IS
    'Persiste de forma atómica el resultado de una llamada: resuelve/crea el contacto por teléfono y, en su defecto, por correo; upsert de call_log (incluye sentiment, migración 49); upsert de lead (incluye source/source_detail de atribución de origen, migración 41); e inserción idempotente de usage_events. '
