...
-- =============================================================================
-- Corrige un error introducido por la migración 26: `INSERT ... ON CONFLICT
-- (organization_id, phone_e164) DO UPDATE` (copiado tal cual de la
-- migración 21) falla con 42P10 ("there is no unique or exclusion
-- constraint matching the ON CONFLICT specification") contra el esquema
-- real actual. Verificado: el constraint `ux_contacts_phone` SÍ existe y SÍ
-- rechaza un duplicado plano de (organization_id, phone_e164) con 23505 —
-- pero no en la forma exacta que `ON CONFLICT (organization_id,
-- phone_e164)` puede inferir (probablemente un índice único parcial o con
-- expresión, agregado junto con `contacts.archived_at`/`phone_e164`
-- nullable por la migración no documentada — no se pudo confirmar la forma
-- exacta sin acceso a introspección SQL directa contra la base).
--
-- En vez de adivinar la forma exacta del índice para que `ON CONFLICT`
-- pueda inferirlo, se usa el patrón estándar de Postgres que no necesita
-- conocerla: intentar el INSERT y capturar `unique_violation`, re-consultando
-- en ese caso. Funciona contra cualquier forma de constraint único que
-- cause el conflicto (parcial, con expresión, o simple), y es el mismo
-- resultado observable (misma fila devuelta, mismos datos actualizados)
-- que el `ON CONFLICT ... DO UPDATE` que reemplaza.
-- =============================================================================
...
BEGIN
    ...
    --    correo después (débil), y el caso nuevo de solo-correo (migración
    --    26). El INSERT del caso "teléfono nuevo" ya no usa
    --    `ON CONFLICT (organization_id, phone_e164)` (ver cabecera) — usa
    --    captura de `unique_violation` en su lugar.
    IF p_caller_phone_e164 IS NOT NULL OR NULLIF(p_email, '') IS NOT NULL THEN
        ...
        ELSIF p_caller_phone_e164 IS NOT NULL THEN
            ...
            EXCEPTION WHEN unique_violation THEN
                -- Carrera real (dos llamadas concurrentes del mismo teléfono
                -- nuevo): otra transacción ganó, se recupera su fila y se
                -- enriquece igual que el camino de UPDATE de arriba.
                SELECT id INTO v_contact_id
                FROM public.contacts
                WHERE organization_id = p_organization_id AND phone_e164 = p_caller_phone_e164;
            ...
            END;
        ELSE
            -- Solo correo, sin coincidencia previa: se crea con phone_e164
            -- NULL. Sin índice único sobre (organization_id, email), no hay
            -- conflicto que capturar aquí.
            INSERT INTO public.contacts (
                organization_id, phone_e164, full_name, email,
                business_name, business_sector, last_seen_at, last_activity_at
            )
...
COMMENT ON FUNCTION public.process_call_completed IS
    'Persiste de forma atómica el resultado de una llamada: resuelve/crea el contacto por teléfono y, en su defecto, por correo (deduplicación cross-canal, migración 21), incluyendo el caso de solo-correo sin teléfono (migración 26, widget web); el insert de contacto nuevo por teléfono captura unique_violation en vez de depender de ON CONFLICT con una forma de índice específica (migración 27); upsert de call_log (incluye channel, dirección de servicio y coordenadas geocodificadas); upsert de lead (DO UPDATE con COALESCE); e inserción idempotente de usage_events. '
